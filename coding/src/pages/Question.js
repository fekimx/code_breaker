...
class Question extends React.Component {
    ...
    .then((res) => {
      ...
      for (let i = 0; i < res['data']['unit_test_results'].length; i++) {
        ...
        if (result) {
          ...
          if (unitTest.visible) {
            unitTests.push(<div style={{backgroundColor: "#F09CA4"}} key={i}><strong>Input:</strong> {unitTest.input} <strong>Expected Output:</strong> {unitTest.expectedOutput}</div>);
          } else {
            unitTests.push(<div style={{backgroundColor: "#F09CA4"}} key={i}>Hidden Unit Test #{i}</div>);
          }
        }
      }
  ...
  render() {
    ...
    <Modal
      ...
      <div style={{backgroundColor: "#F09CA4"}}>
        {this.state.stderr}
      </div>
      </div>
      </div>
